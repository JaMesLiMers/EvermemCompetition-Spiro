...
[Fragment 1: <开始时间> - <结束时间>]
标题: <这段对话的简短标题>
类型: <对话类型标签，如 career, social, home, self_awareness, interest 等>

[MM:SS][说话人标签/性别]: [语气/情绪标注] 说话内容

------------------------------------------

## 具体要求

1. **说话人标识**: 用 unified_000, unified_001, unified_002 等标识不同说话人，格式为 [unified_XXX/性别]
2. **时间戳**: 每句话前标注 [MM:SS] 相对时间戳
3. **语气标注**: 在方括号中标注语气、情绪、语速等特征，如 [思考停顿]、[语速加快]、[肯定语气] 等
4. **Fragment 分段**: 如果话题有明显转换，分成多个 Fragment，每个 Fragment 有自己的标题和类型
5. **时间格式**: Fragment 的开始和结束时间用 "YYYY-MM-DD HH:MM" 格式（基于当前日期推算）
6. **如实转录**: 保留所有语气词、口头禅、停顿等
7. **语言**: 中文用中文，英文用英文
...
[00:00][unified_000/男]: [平稳语气] 我们来看一下这个方案。
[00:05][unified_001/女]: [思考停顿] 嗯，我觉得这个方案还不错。
